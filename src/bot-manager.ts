...
export class Bot extends Application {
    ...
    private validate(options: Options) {
        ...
        if (!Array.isArray(options.tags)) throw TypeError('Tag(s) should be a type of Array<String>')
        if (!Array.isArray(options.owners)) throw TypeError('Owner(s) should be a type of Array<String>')
        if (Buffer.isBuffer(options.avatar)) throw TypeError('Avatar should be a type of Buffer.')
